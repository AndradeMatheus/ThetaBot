...
export default class CustomCommandService implements ICustomCommandService {
  ...
  private myInstantsSlashCommand = container.resolve<MyInstantsSlashCommand>(Tokens.MyInstantsSlashCommand);
  private imageSlashCommand = container.resolve<ImageSlashCommand>(Tokens.ImageSlashCommand);
  ...
  async handle(
    ...
    // get command from db
    const command = await this.commandsRepository.getCommandByAlias(guildId, commandName);
    ...
    const actions = new Map<'img' | 'inst', (fnInteraction: CommandInteraction, fnCommand: ICommand) => Promise<void>>();
